<template>
    <div class="signup-buttons">
        <div id="fb-root"></div>
        <a href="#" class="google-signup" @click="googleLogin">
        ...
        <a href="#" class="facebook-signup" @click="facebookLogin">
...
export default {
  ...
  methods: {
    ...
    googleLogin() {
      ...
      const infor = firebase.auth().signInWithPopup(provider)
        .theh( function (result) {
            var token = result.credential.accessToken
            var user = result.user
            alert("user : " + user)
            console.log("user : " + user)
